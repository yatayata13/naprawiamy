...
case "$SITE" in
    naprawiamy | fixmystreet | mapit | theyworkforyou | pombola | alaveteli)
        if [ $DOCKER = true ] && [ "$SITE" != 'naprawiamy' ]; then
...
if [ $SITE = 'naprawiamy' ]; then
    REPOSITORY_URL=${REPOSITORY_URL_OVERRIDE:-https://github.com/yatayata13/naprawiamy.git}
else    
